<template>
  ...
  <div class="ordered-grid">
    <div v-for="(a, k) in question.a" v-bind:class="['answer', { wiggle: currentlyMoving === a }]" draggable="true"
      v-on:mousedown="prepareDrag(a)" v-on:dragstart="startDrag($event)" v-on:dragenter="updatePosition($event, k)"
      v-on:dragend="drop" v-bind:key="a.text" v-bind:ref="a.text">
      {{ a.text }}
    </div>
  </div>
</template>
